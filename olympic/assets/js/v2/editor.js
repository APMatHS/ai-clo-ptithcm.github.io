function lessonEditor(c,subject,topics,x,code){
  ...
  $('#texToolbar').onclick=e=>{const b=e.target.closest('button');if(!b)return;let sn=b.dataset.snippet||'',env=b.dataset.env;if(env)sn=`\\begin{${env}}\nNội dung ${env}.\n\\end{${env}}`;insertAtCursor(ta,sn);draw();state.dirty=true};
