async function renderLessons(c){
  ...
  if(id){
    ...
    const x=r.data;
    let tq=db.from('olympic_tests').select('id,title,description,status').eq('subject_id',state.subject.id).eq('lesson_id',x.id).eq('is_visible',true);
    if(!staff())tq=tq.eq('status','published');
    const tr=await tq.order('order_index').limit(1).maybeSingle();
    if(tr.error)console.warn('Olympic lesson test:',tr.error);
    const test=tr.data;
    c.innerHTML=`<div class="oly-toolbar"><a class="oly-btn" href="${subjectFeatureUrl('lessons')}">← Danh sách bài học</a></div><article class="oly-panel"><span class="oly-badge published">${statusLabel(x.status)}</span><h2 style="font-family:Cambria Math,Georgia,serif;font-size:30px;margin-bottom:6px">${esc(x.title)}</h2><p style="color:#627d98">${esc(x.summary||'')}</p>${x.topic?.title?`<div class="meta"><span class="oly-badge">${esc(x.topic.title)}</span></div>`:''}<div class="oly-preview" style="border:0;padding:10px 0 0;min-height:0" id="lessonBody">${renderTex(x.content_tex)}</div>${test?`<section class="oly-card" style="margin-top:22px"><div class="math-icon green">π</div><h3>Đề luyện cuối bài</h3><p>${esc(test.description||'Củng cố ngay các kỹ thuật vừa học.')}</p><div class="meta"><span class="oly-badge">Lộ trình</span>${staff()?`<span class="oly-badge ${esc(test.status)}">${statusLabel(test.status)}</span>`:''}</div><a class="oly-btn primary" style="margin-top:10px" href="${subjectFeatureUrl('tests')}?id=${encodeURIComponent(test.id)}">Làm đề luyện →</a></section>`:''}</article>`;
    return;
  }
