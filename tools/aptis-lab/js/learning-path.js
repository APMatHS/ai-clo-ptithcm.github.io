(() => {
  ...
  function optionsHtml(q) {
    ...
    if (options.length) return `<div class="options">${options.map(o => { const selected = q.response?.value === o.key; const correct = q.answered && q.correct_answer?.value === o.key; const wrong = q.answered && selected && q.is_correct === false; return `<button type="button" class="option-btn ${correct ? 'correct' : ''} ${wrong ? 'wrong' : ''}" data-lp-option="${esc(o.key)}" ${q.answered ? 'disabled' : ''}><span class="option-key">${esc(o.key)}</span><span>${esc(o.text)}</span></button>`; }).join('')}</div>`;
