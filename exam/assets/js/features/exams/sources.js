...
import { escapeHtml,formatDateTime,toast,errorMessage,confirmDialog,setBusy } from '../../core/ui.js';
...
export async function renderSources(ctx){
  ...
  host.innerHTML=`<div class="card"><div class="toolbar"><div><div class="card-title">Tài liệu đề gốc</div><div class="muted">Lưu Word/PDF do giảng viên hoặc khảo thí cung cấp để đối chiếu với bản thi đã biên tập.</div></div>${can?'<label class="btn btn-primary source-upload">+ Tải Word/PDF<input data-source-file type="file" accept=".docx,.pdf" hidden></label>':''}</div>
  ...
  input?.addEventListener('change',async()=>{const file=input.files?.[0];if(!file)return;const label=input.closest('.source-upload');setBusy(label,true);try{await storageService.upload({examId:exam.id,file,kind:'source',metadata:{purpose:'original_exam_source'}});toast('Đã lưu tài liệu gốc.','success');await renderSources(ctx);}catch(e){toast(errorMessage(e),'error');setBusy(label,false);}});
