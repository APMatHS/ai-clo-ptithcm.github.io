...
export async function renderSources(ctx){
  ...
  input?.addEventListener('change',async()=>{const file=input.files?.[0];if(!file)return;const label=host.querySelector('[data-upload-label]');input.disabled=true;if(label)label.textContent='Đang tải…';try{await storageService.upload({examId:exam.id,file,kind:'source',metadata:{purpose:'original_exam_source'}});toast('Đã lưu tài liệu gốc.','success');await renderSources(ctx);}catch(e){toast(errorMessage(e),'error');input.disabled=false;if(label)label.textContent='+ Tải Word/PDF/TEX';}});
