...
const tabs=[['overview','Tổng quan'],['sessions','Ca & phòng'],['roster','Danh sách SV'],['paper','Đề thi'],['members','Nhân sự']];
...
export async function renderExamDetail(root,profile,examId){
  ...
  const mount=async id=>{saveUiState(key,id);root.querySelectorAll('[data-tab]').forEach(b=>b.classList.toggle('active',b.dataset.tab===id));host.innerHTML='<div class="card">Đang tải…</div>';if(id==='overview')await renderOverview(ctx);if(id==='sessions')await renderSessions(ctx);if(id==='roster')await renderRoster(ctx);if(id==='paper')await renderPaper(ctx);if(id==='members')await renderMembers(ctx);};
