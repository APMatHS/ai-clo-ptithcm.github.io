...
export async function renderExamDetail(root,profile,examId){
  ...
  const tabs=allTabs.filter(([id])=>id==='overview'||id==='sessions'||(id==='roster'&&hasAnyPermission(profile,rawMembership,'manage_roster'))||((id==='paper'||id==='import')&&hasAnyPermission(profile,rawMembership,'manage_paper'))||(id==='sources'&&hasAnyPermission(profile,rawMembership,'manage_assets'))||(id==='members'&&hasAnyPermission(profile,rawMembership,'manage_members'))||(id==='preflight'&&canFullPreflight));
