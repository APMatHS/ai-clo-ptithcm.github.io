...
export async function saveExamMember(examId,userId,examRole,permissions=[]){const {data,error}=await supabase.from('exam_members').upsert({exam_id:examId,user_id:userId,exam_role:examRole,permissions},{onConflict:'exam_id,user_id'}).select().single();fail(error);return data;}
