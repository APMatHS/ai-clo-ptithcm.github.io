...
as $$
declare
  st public.exam_students%rowtype;
  exam_state public.exam_status;
  paper_session uuid;
  paper_state public.paper_status;
begin
  ...
  if new.status in ('in_progress','locked') and exam_state <> 'live' then
...
create trigger guard_exam_attempt_scope
before insert or update of exam_student_id,session_id,room_id,paper_version_id,status
