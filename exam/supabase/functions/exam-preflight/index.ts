...
async function runPreflight(examId:string){
  const issues:any[]=[];
  ...
  for(const session of sessions){
    ...
    const vr=await db.from('exam_paper_versions').select('*').eq('paper_id',paper.id).in('status',['locked','hotfix']).order('version_no',{ascending:false}).limit(1).maybeSingle();if(vr.error)throw vr.error;const version=vr.data;
    if(!version){issues.push(issue('error','PAPER_NOT_LOCKED',`${session.name}: đề chưa có phiên bản locked/hotfix.`,{sessionId:session.id,paperId:paper.id}));sessionReport.push({sessionId:session.id,name:session.name,rooms:sr.length,students:ss.length,questionCount:0,points:0,paperStatus:'draft'});continue;}
...
Deno.serve(async req=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors(req)});if(req.method!=='POST')return out(req,{error:'Method not allowed'},405);try{const b=await req.json().catch(()=>({})),examId=String(b.examId||'');if(!examId)return out(req,{error:'Thiếu examId.'},400);const a=await actor(req);if(!await canExam(a,examId))return out(req,{error:'Bạn không có quyền kiểm tra kỳ thi này.'},403);return out(req,await runPreflight(examId));}catch(e){console.error(e);const s=Number((e as any)?.status||500);return out(req,{error:s>=500?'Máy chủ đang gặp lỗi.':String((e as any)?.message||'Có lỗi xảy ra.')},s);}});
