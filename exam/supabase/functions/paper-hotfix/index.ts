...
async function canManage(a:any,examId:string){if(a.profile.system_role==='admin')return true;const {data,error}=await db.from('exam_members').select('exam_role,permissions').eq('exam_id',examId).eq('user_id',a.user.id).maybeSingle();if(error)throw error;return !!data&&(data.exam_role==='owner'||(data.permissions||[]).includes('manage_paper'));}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    const a=await actor(req);if(!await canManage(a,examId))return out(req,{error:'Bạn không có quyền sửa đề.'},403);
    ...
    await db.from('audit_logs').insert({actor_user_id:a.user.id,exam_id:examId,action:'paper_hotfix',entity_type:'question',entity_id:questionId,payload:{newQuestionVersionId,newPaperVersionId,readyAttemptsUpdated:ready.data?.length||0,unseenAttemptsUpdated:updated,alreadySeenKeptOld:seen}});
