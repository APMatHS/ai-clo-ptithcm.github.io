  ...
  properties: {
    ...
    scores: {
      ...
      properties: {
        ...
        fluency_coherence: { type: "integer", minimum: 0, maximum: 5 },
        pronunciation_intelligibility: { type: "integer", minimum: 0, maximum: 5 },
      },
      required: ["task_fulfilment", "grammar", "vocabulary", "fluency_coherence", "pronunciation_intelligibility"],
    },
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: cached } = await db.from("aptis_ai_assessments").select("*")
      ...
      .eq("assessment_type", "speaking").maybeSingle();
    ...
    if (!item?.answered_at || !question || question.skill !== "speaking") return json(req, { ok: false, error: "Speaking response not found" }, 400);

    const recording = item.response?.recording;
    if (!recording?.path) return json(req, { ok: false, error: "Recording not found" }, 400);
    const bucket = recording.bucket || "aptis-recordings";
    const { data: blob, error: downloadError } = await db.storage.from(bucket).download(recording.path);
    if (downloadError || !blob) throw new Error(downloadError?.message || "Cannot load recording");
    if (blob.size > 18 * 1024 * 1024) return json(req, { ok: false, error: "Recording is too large for inline assessment" }, 413);

    const { error: creditError } = await userDb.rpc("aptis_take_ai_credit", { p_feature: "speaking_assessment" });
    if (creditError) return json(req, { ok: false, error: creditError.message || "AI quota exceeded" }, 429);

    const bytes = new Uint8Array(await blob.arrayBuffer());
    const mime = recording.mime || blob.type || "audio/webm";
    const prompt = [
      "You are an English speaking practice assessor for Aptis General preparation.",
      "Evaluate only the response you can actually hear. Do not invent missing words.",
      ...
      "If the recording is too short, silent, or unintelligible to assess, use status=insufficient_evidence and conservative scores.",
      "Keep strengths, improvements and the next-attempt tip concise and actionable.",
    ].join("\n");
    ...
    let rubric: any = null;
    try { rubric = JSON.parse(text); } catch { throw new Error("Gemini returned invalid assessment JSON"); }

    const total = ["task_fulfilment", "grammar", "vocabulary", "fluency_coherence", "pronunciation_intelligibility"]
    ...
    console.error("aptis-assess-speaking", error);
