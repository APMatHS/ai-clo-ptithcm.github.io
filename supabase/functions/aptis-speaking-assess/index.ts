...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: item, error: itemError } = await db
      ...
      .select("id,response,answered_at,attempt_id,question_id,aptis_attempts!inner(user_id,mode,completed_at),aptis_questions!inner(skill,part,prompt,content,question_type)")
      .eq("attempt_id", attempt_id)
      .eq("question_id", question_id)
      .eq("aptis_attempts.user_id", uid)
      .single();

    if (itemError || !item) return json(req, { ok: false, error: "Speaking response not found" }, 404);

    const question: any = (item as any).aptis_questions;
    const attempt: any = (item as any).aptis_attempts;
    if (question?.skill !== "speaking" || question?.question_type !== "speaking_prompt") {
      return json(req, { ok: false, error: "Not a speaking prompt" }, 400);
    }
    ...
    const media = (item as any).response?.recording;
    if (!media?.bucket || !media?.path) {
      return json(req, { ok: false, error: "Speaking recording is missing" }, 400);
    }

    const { data: file, error: downloadError } = await db.storage
      .from(media.bucket)
      .download(media.path);
    if (downloadError || !file) throw new Error(downloadError?.message || "Cannot read recording");
    if (file.size < 2500) {
      return json(req, { ok: false, error: "Recording is too short to assess" }, 422);
    }

    const audio = bytesToBase64(new Uint8Array(await file.arrayBuffer()));
    const instruction = `You are an English speaking practice assessor for a learner preparing for Aptis General around B1-B2. This is formative practice, not an official Aptis score. Evaluate the actual audio, not only a transcript. Prompt: "${question.prompt}". Part: ${question.part}. Return ONLY valid JSON with this exact shape:\n{"status":"completed|insufficient_evidence","estimated_level":"B1|B1+|B2|B2+|null","scores":{"task_fulfilment":0,"grammar":0,"vocabulary":0,"fluency_coherence":0,"pronunciation_intelligibility":0},"strengths":[""],"improvements":[""],"notable_errors":[{"heard":"","suggestion":""}],"next_attempt_tip":"","transcript":""}\nEach score is an integer 0-5. Use insufficient_evidence if audio is too short, unintelligible, or has too little English to assess. Do not infer proficiency from accent. Pronunciation means intelligibility, stress/rhythm and clarity. Keep feedback concise, supportive and specific.`;
    ...
    const geminiResponse = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent`,
      {
        ...
        body: JSON.stringify({
          contents: [{ parts: [
            { text: instruction },
            { inlineData: { mimeType: file.type || media.mime || "audio/webm", data: audio } },
          ] }],
          generationConfig: {
    ...
    const { data: featureUsage } = await db
      ...
      .eq("user_id", uid)
      .eq("usage_date", today)
      .eq("feature", "speaking_assessment")
      .maybeSingle();
    ...
    console.error("aptis-speaking-assess", error);
    return json(req, {
      ok: false,
